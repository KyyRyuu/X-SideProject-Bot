...
export class Store {
  constructor(options = {}) {
    ...
    // Contact collection/persistence lives in src/fetchContact.js. Inject a
    // shared instance via options.contacts; otherwise create a local one.
    this.contactBook =
      options.contacts || createContactBook({ contactsPath: options.contactsPath, logger: this.logger });
  }

  /** Backward-compatible view of contacts (Map keyed by phone/lid JID). */
  get contacts() {
  ...
  bind(ev) {
    // App-state/history sync deliver the saved address book here.
    ev.on("messaging-history.set", ({ contacts = [], chats = [] }) => {
      for (const c of contacts) this.contactBook.upsert(c);
      for (const c of chats) this.chats.set(normalize(c.id), c);
      this.logger?.info(`history sync: ${contacts.length} contacts, ${chats.length} chats`);
    });
