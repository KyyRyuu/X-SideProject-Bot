...
export function createHandler(deps) {
  ...
  return async function handle(raw) {
    ...
    try {
      ...
      let { prefix, command, args, text } = parseCommand(m.text, settings.prefix);
      let plugin = command !== null ? registry.find(command) : null;

      // Prefixless sigil commands (e.g. "$ ls", "=> 1+1"). Owner-only sigils are
      // skipped silently for non-owners so innocent messages aren't answered.
      if (!plugin) {
        const np = matchNoPrefix(m.text, registry, owner);
        if (!np) return;
        ({ prefix, command, args, text, plugin } = np);
      }
...
/**
 * Resolve a prefixless sigil command from the raw body. Returns the same shape
 * as parseCommand plus the matched plugin, or null when nothing matches.
 */
function matchNoPrefix(body, registry, owner) {
  if (!body) return null;
  for (const { cmd, plugin } of registry.noPrefixCommands()) {
    if (!body.startsWith(cmd)) continue;
    if (plugin.owner && !owner) continue;
    const rest = body.slice(cmd.length).replace(/^\s+/, "");
    const args = rest ? rest.split(/\s+/) : [];
    return { prefix: "", command: cmd, args, text: rest, plugin };
  }
  return null;
}
