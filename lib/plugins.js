...
export class PluginRegistry {
  ...
  /**
   * Commands that may be invoked without a prefix, longest first. A plugin opts
   * in via `noPrefix: true` (all its commands) or `noPrefix: ["$"]` (a subset).
   * Restricting to a subset keeps word commands like "exec"/"sh" prefixed while
   * exposing only the sigil ("$").
   * @returns {Array<{ cmd: string, plugin: object }>}
   */
  noPrefixCommands() {
    const out = [];
    for (const plugin of this.byPath.values()) {
      if (!plugin.noPrefix) continue;
      const tokens = Array.isArray(plugin.noPrefix) ? plugin.noPrefix : this._commands(plugin);
      for (const cmd of tokens) out.push({ cmd: String(cmd).toLowerCase(), plugin });
    }
    return out.sort((a, b) => b.cmd.length - a.cmd.length);
  }
