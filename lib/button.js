...
export function bindButton(sock) {
  sock.sendButton = async (jid, content = {}, options = {}) => {
    if (content.interactiveMessage) content = content.interactiveMessage;
    ...
    if (Array.isArray(content.cards) && content.cards.length > 0) {
      ...
      const carousel = {
        header: { title: content.header || content.title || "" },
    ...
    const buttons = normalizeButtons(rawNativeFlow?.buttons || content.buttons || content.interactiveButtons || []);
    let nativeFlow = { buttons };
    if (rawNativeFlow) {
      const { buttons: _ignored, ...rest } = rawNativeFlow;
      nativeFlow = { ...nativeFlow, ...rest };
    }

    const interactive = {
      header: { title: content.header || content.title || "", subtitle: content.subtitle || "", ...header },
      body: { text: content.body || content.text || content.caption || "" },
      footer: { text: content.footer || "" },
